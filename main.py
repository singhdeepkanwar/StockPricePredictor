...
y = np.where((data['Close'].shift(-1) > data['Close']) & (data['EMA_9'] > data['EMA_21']), 1, -1)
...
for i in range(len(data)):
    if data['EMA_9'].iloc[i] > data['EMA_21'].iloc[i] and position == 0:
        ...
        if data['Close'].iloc[i] <= buy_price * 0.95:
            ...
            print(f"Stop-loss hit! Sell at {data['Close'].iloc[i]:.2f} on {data['Date'].iloc[i].date()}")
        ...
        # Check for regular EMA crossover sell signal
            ...
            stock_value = 0
            position = 0  # Position closed
            print(f"Sell at {data['Close'].iloc[i]:.2f} on {data['Date'].iloc[i].date()}")
